...
client.on('connect', () => {
  ...
  client.subscribe(topic, (err) => {
    if (!err) {
      ...
      // const message = {
      //   deviceId: 'sensor_001',
      //   temperature: Math.random() * 30 + 10, // 模拟温度数据（10-40℃）
...
client.on('message', (topic, payload) => {
  ...
  if (message.deviceId === 'sensor_001') {
    // 发布消息到该主题
    const message = {
      deviceId: 'sensor_001',
      temperature: Math.random() * 30 + 10, // 模拟温度数据（10-40℃）
    ...
    // 发布消息（主题，消息内容（需转字符串），QoS 等级）
    client.publish('device_topic', JSON.stringify(message), { qos: 1 }, (err) => {
      if (err) {
        console.error('❌ 发布消息失败：', err);
      } else {
        console.log('📤 发布消息：', message);
